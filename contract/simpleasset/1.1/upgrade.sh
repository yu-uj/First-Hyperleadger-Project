...
#1. 설치
docker exec cli peer chaincode install -n simpleasset -v 1.1.1.1 -p github.com/simpleasset/1.1

#2. 업그레이드
docker exec cli peer chaincode upgrade -n simpleasset -v 1.1.1.1 -c '{"Args":[]}' -C mychannel -P 'AND("Org1MSP.member")'
sleep 3

#3. 인보크 a, b, transfer
docker exec cli peer chaincode invoke -n simpleasset -C mychannel -c '{"Args":["get","c","300"]}'
